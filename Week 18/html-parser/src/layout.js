...
function layout(element) {
    ...
    if (elementStyle.display !== 'flex')    // 目前只处理display为flex的情况
        return;
    ...
    if (style.flexDirection === 'row') {
        ...
        mainStart = 'left';
        mainEnd = 'right';
        mainSign = +1;      // 符号，从左往右排为正
        mainBase = 0;       // 初始值，和mainSize是一对
    ...
    flexLine.mainSpace = mainSpace; // 每行的剩余空间
    // console.log(items);
    ...
    if (mainSpace < 0) {
        ...
        flexLines.forEach(function (items) {
            ...
            if (flexTotal > 0) {
                ...
                if (style.jstifyContent === 'space-between') {
